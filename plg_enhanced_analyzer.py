...
class EnhancedVerdict:
    ...
    fundamental_verdict: str  # From thesis logic
...
def analyze_valuation(
    ...
    # Benchmark P/S ratios for SaaS (from thesis)
    # Premium: 15-25x (NDR >120%, growth >30%)
    # Fair: 10-15x (NDR 110-120%, growth 20-25%)
    # Value: <10x (but often for good reason)
    ...
    if price_data.pct_off_high is not None:
        # Stock far from highs = opportunity if fundamentals strong
    ...
    if fundamental_verdict in ['STRONG_BUY', 'BUY']:
        if valuation_tier in ['cheap', 'fair']:
            if price_data.pct_off_high and price_data.pct_off_high < -20:
                timing_signal = 'buy_now'  # Strong fundamentals, cheap, beaten down
            elif price_data.rsi_14 and price_data.rsi_14 < 40:
                timing_signal = 'buy_now'  # Oversold
            else:
                timing_signal = 'accumulate'  # Good price
        else:
            timing_signal = 'wait_for_pullback'  # Too expensive
    
    elif fundamental_verdict == 'WATCH':
        if valuation_tier == 'cheap' and price_data.pct_off_high and price_data.pct_off_high < -25:
            timing_signal = 'monitor_closely'  # Might be opportunity
        else:
            timing_signal = 'monitor'
    
    else:  # SELL or AVOID
        if price_data.return_3m and price_data.return_3m > 10:
            timing_signal = 'sell_rally'  # Exit while you can
        else:
            timing_signal = 'avoid'
...
def compute_enhanced_verdict(
    ...
    growth_str = f"{revenue_growth*100:.0f}%" if revenue_growth and revenue_growth < 1 else f"{revenue_growth:.0f}%" if revenue_growth else "N/A"
...
def analyze_company_enhanced(ticker: str, company_data: dict) -> dict:
    """
    ...
    # Get fundamental data (simplified - use your existing logic)
    ndr = company_data.get('ndr')
    revenue_growth = company_data.get('revenue_growth_yoy')
    category_stage = company_data.get('category_stage', 'mid_growth')
    
    # Mock fundamental verdict (replace with your actual verdict logic)
    if ndr and ndr >= 120 and revenue_growth and revenue_growth >= 0.25:
        fundamental_verdict = 'STRONG_BUY'
        confidence = 'HIGH'
    elif ndr and ndr >= 110 and revenue_growth and revenue_growth >= 0.20:
        fundamental_verdict = 'BUY'
        confidence = 'MEDIUM'
    elif ndr and ndr < 100:
        fundamental_verdict = 'SELL'
        confidence = 'MEDIUM'
    else:
        fundamental_verdict = 'WATCH'
        confidence = 'MEDIUM'
    ...
    return {
        ...
        # Fundamentals
        'ndr': ndr,
        'revenue_growth_pct': revenue_growth * 100 if revenue_growth and revenue_growth < 1 else revenue_growth,
...
if __name__ == "__main__":
    ...
    for ticker, data in test_companies.items():
        result = analyze_company_enhanced(ticker, data)
    ...
    print(f"{'Ticker':<8} {'Score':<7} {'Recommendation':<25} {'P/S':<7} {'vs 52w High':<12}")
    print("-" * 70)
    ...
    for r in results:
        ...
        print(f"{r['ticker']:<8} {r['opportunity_score']:<7.0f} {r['final_recommendation']:<25} {ps_str:<7} {off_high_str:<12}")
